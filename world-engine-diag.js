...
window.WORLD_ENGINE_DIAG = (function() {
  ...
  function collectMemory() {
    ...
    return {
      ...
      extraction: safe(function () { return { isRunning: engine?.isRunning?.() ?? null, lastError: debug?.error || null, lastStatus: Number.isFinite(Number(debug?.status)) ? Number(debug.status) : null, lastPrompt: debug?.prompt || debug?.requestPrompt || '', lastRawResult: debug?.rawResult || debug?.apiResponse || debug?.response || '' }; }),
