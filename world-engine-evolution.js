...
### 输出字数限制
以下限制按中文汉字计数，标点符号、空格、数字和英文字母不计入字数。**每条文字描述不得少于120个汉字**，通常写到150-200字：写清具体细节、因果与影响，禁止只写一句话或干巴巴的概括。参考下方 JSON 示例的详细程度。
- events.desc：120-200个汉字（描述事件当前进展、关键细节与影响）。
- factions.currentGoal：120-200个汉字（写明势力当前目标与推进情况）。
- winds.content：120-200个汉字（写明正在传播的具体说法、关键细节与圈层反应）。
- influenceChain.impact：120-200个汉字。
- influenceChain.fallout：120-200个汉字。
- worldTrends.description：120-200个汉字。
...
`;
  ...
  const JSON_EXAMPLE = `{
  "events": [
    { "id": "event_1", "name": "血刀门复仇", "type": "conflict", "level": 2, "stage": "发酵", "stageRound": 5, "desc": "血刀门派出了三名追踪者，沿青石关官道一路向北搜索{{user}}的踪迹，并在三里亭设下暗哨昼夜轮换。他们持有血刀门信物，可调用沿途的线人与客栈伙计提供情报，青石镇上的药铺、车马行近日都被问过话。血刀老祖放话活要见人死要见尸，赏格抬到了三百两，草莽中已有人蠢蠢欲动想赚这笔钱。" },
    { "id": null, "name": "青炉司改良火药", "type": "progress", "level": 3, "stage": "执行", "stageRound": 4, "desc": "青炉司已收齐硝石、密炭与硫磺，三座试验小炉全部点火成功，第二批配比正在试烧。匠头赵四提出用石灰岩粉替代部分硝石以降低成本，已试制二十斤，威力较旧方略弱但价格仅三分之二，正在权衡是否投入量产。朝廷火器局上月来信询问进度，若能赶在秋防前拿出稳定配方，青炉司可拿到三年专供契。" }
  ],
  "factions": [
    { "id": "faction_1", "name": "血刀门", "scope": "血刀岭及周边三镇", "status": "稳固", "relation": "敌对", "currentGoal": "全力缉拿杀死少主的凶手，同时借此事震慑周边山寨，巩固血刀岭对青石关至飞狐口商路的控制权；若三月内无果，将转向与北地马匪结盟以扩大搜捕范围", "core_person": "血刀老祖", "powerPillars": ["武力威慑","情报网"] }
  ],
  "worldTrends": [
    { "id": "trend_1", "name": "北境战争", "scope": "北境三州及周边诸国", "status": "持续中", "description": "边军与北境诸部的战争进入第三个年头，青壮年大量应征导致边地田亩抛荒，粮价三年涨了近两倍。朝廷为筹措军费加征盐税与厘金，波及内地商路，各地镖局人手紧张、护卫费用水涨船高。北境诸部在秋收前频频袭扰边境粮道，边军主力被牵制在雁门一线，短期内无力他顾，这场战争正在重塑整个北方的经济与势力格局。", "source": "Lv4冲突型事件「北境战争」进入已爆发" }
  ],
  "winds": [
    { "id": "wind_1", "topic": "青石关设卡", "type": "report", "level": 2, "content": "青石关北门昨日新增了官兵哨卡，盘查过往行人商旅，重点比对画像上的人像。据说守备大人收到了上峰密令，要缉拿一名朝廷要犯，但具体罪名讳莫如深。商队私下议论这是冲着江湖人来的，不少带刀佩剑的客商已经改走水路绕行，青石关客栈的生意一夜冷清了不少。", "scope": "青石关及周边村镇", "source": "目击商贩→往来商队" }
  ],
  "economy": { "climate": "平稳", "signals": [] },
  "reputation": { "authority": "默默无闻", "common": "默默无闻", "shadow": "默默无闻", "circuit": "默默无闻", "lastChange": "无变化" },
  "world_digest": "血刀门追踪者在青石关外三里亭设了暗哨，青石关北门新增盘查哨卡，商旅纷纷改道；天机阁阁主上官云密信召回了三名外围密探；醉仙楼后厨因粮商涨价换了供货渠道，市井间传言北境战事还要再打两年。",
  "enemies": [
    { "id": "enemy_1", "name": "血刀门", "reason": "{{user}}杀了血刀门少主", "type": "blood", "status": "执行中" }
  ],
  "influenceChain": [
    { "trigger": "血刀门发布悬赏令", "impact": "草莽中人开始主动留意{{user}}的行踪，青石关一带的客栈伙计、脚夫与赌坊耳目都记住了画像上的形貌特征，消息正沿商路向周边三镇扩散", "fallout": "客栈与渡口出现试探和秘密报信者，{{user}}若公开露面很可能被认出并遭到跟踪；血刀门的眼线网络因此扩大，未来缉拿行动将更加精准" }
  ],
...
【思考要求】先以 <thinking>...</thinking> 标签简要写下本轮推演的思考过程（关键判断依据、因果链、信息传播边界、取舍理由，100-300字），再输出最终 JSON。思考内容仅供人类查看，不得影响 JSON 的正确性；最终 JSON 必须完整、独立、可解析。`;
